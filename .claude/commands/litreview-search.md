...
# narrower bound would silently and permanently miss a late-indexed paper.
# tools/dedup.py's record_id idempotency (below) is what keeps a living
# rerun cheap in *new records*, even though it isn't cheap in API calls.
SINCE="$(python3 -c "
import json
try:
