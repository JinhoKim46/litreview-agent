...
3. **`"classification"`** (`methods/systematic_mapping_study.json`): do not run any of this file's remaining steps. Load and follow `.claude/skills/study-classification/SKILL.md` instead — it builds `classification_table.json` with its own keyword/freeze/code/calibrate workflow. Tell the reviewer plainly that this review is a systematic mapping study and classification (not extraction) is the applicable stage, then hand off to that skill for the rest of this run.
4. **`"relevance_tags"`** (`methods/reconnaissance.json`): do not run any of this file's remaining steps. Load and follow `.claude/skills/reconnaissance-brief/SKILL.md` instead — it builds `relevance_tags_table.json` with its own tag/claim workflow (no freeze gate at all — a reconnaissance brief has no co-developed form to freeze). Tell the reviewer plainly that this review is an exploratory literature brief and relevance-tagging (not extraction) is the applicable stage, then hand off to that skill for the rest of this run.
5. **Any other value**: **stop the entire command.** No skill or command supports this capture stage yet — tell the reviewer plainly which method this review uses and that no command supports its capture stage yet, pointing at `docs/ROADMAP.md`'s method taxonomy rather than implying one exists. Never attempt to "extract" from a review whose capture stage this command doesn't recognize anyway just because the reviewer wants a table.
...
Study design decides which risk-of-bias tool applies in Step 7 — RoB1 for randomized trials, the Newcastle-Ottawa Scale (NOS) for everything else, and neither for a design with no comparator arm at all. RoB1 and NOS are not interchangeable (RoB1's "allocation concealment" domain is meaningless on a cohort study), and forcing either one onto a design that has no comparator is a fabricated judgement, not a real one. Per `quality-appraisal/SKILL.md`, confirm this explicitly rather than inferring it silently:
...
2. Otherwise ask directly: "Is `<record_id>` (`<title>`, `<year>`) a randomized controlled trial, a non-randomized study (cohort, case-control, before-after, cross-sectional), or a design with no comparator arm at all (diagnostic test accuracy, single-arm trial, case series/case report, qualitative study)?" Use `AskUserQuestion` with those three as bounded options plus "not sure yet — I'll check the full text."
3. Record the confirmed design as `study_design` for this study (free text is fine, e.g. `"RCT, parallel-group"`, `"prospective cohort"`, `"single-arm trial"`) and note internally whether Step 7 will run RoB1, NOS, or fail closed as `"unsupported"` (no comparator arm — see `quality-appraisal/01-risk-of-bias.md` Part 3). If the reviewer picked "not sure yet," defer this choice until Step 4's full text is in hand, then confirm before Step 7.
