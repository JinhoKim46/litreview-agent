# /litreview-synthesize - Pool, Assess Heterogeneity, and Grade the Included Studies

Implements architecture plan §6 (the NEW statistical-synthesis stage). Reads `results/<TOPIC>/extraction_table.json` (built by `/litreview-extract`), groups included studies by outcome, pools every outcome group that clears the poolability gate, falls back to narrative synthesis for every group that doesn't — **recording why, never silently** — and writes the four `synthesis/*.json` files plus forest/funnel SVGs that `/litreview-report` reads verbatim.
...
2. Confirm `results/<TOPIC>/extraction_table.json` exists and is non-empty. If it's missing: stop and tell the reviewer to run `/litreview-extract` first — this command never invents study data.
...
4. Confirm `results/<TOPIC>/synthesis_plan.json` exists (schema: `schemas/synthesis_plan.schema.json`). If it's missing, Step 3's script will refuse to run rather than guess a pooling model — the fixed-vs-random decision must be prespecified, never chosen from the data's own heterogeneity statistics. If it's missing, either route the reviewer back to `/litreview-init`'s protocol update path to sign one (preferred — this is what makes the model reportable as prespecified under PRISMA 2020 item 13a-c), or, only for a one-off run, pass `--model {fixed,random}` to Step 3's script explicitly (recorded as `model_source: "cli_override"`, disclosed in the manuscript rather than presented as prespecified).
5. Read `results/<TOPIC>/extraction_table.json` with the `Read` tool (this file is per-study structured data, not a 5,000-record ledger like `records.jsonl` — reading it directly is fine and necessary to sanity- check its shape before Step 3 shells out to it).

**Expected shape** — the exact file `/litreview-extract.md` Step 8 writes, and what `synthesis/run_synthesis.py` expects. If the file you read doesn't match this, stop and fix `extraction_table.json` (or `/litreview-extract` itself) before proceeding — never adapt this command's script to silently tolerate a different shape:
  ...
  "studies": [
    {
      ...
      "risk_of_bias": {"tool": "RoB1", "overall_judgement": "unclear risk", "...": "per quality-appraisal/01-risk-of-bias.md"},
      "effect_data": [
...
Add `--model {fixed,random}` only for the one-off override described in Step 1.4 — the normal path prespecifies the model in `synthesis_plan.json`, not on the command line.

(This exact invocation is pre-allowlisted in `.claude/settings.json` — `Bash(python3 -m synthesis.run_synthesis:*)`. Per PRODUCT_READINESS_AUDIT.md P0-1, the script derives `results/<TOPIC>/extraction_table.json` and `results/<TOPIC>/synthesis/` itself from the validated `<TOPIC>` slug — it no longer accepts free-form `--extraction-table`/`--out-dir` paths, so this pre-approved permission can never be used to write outside a review's own directory. If `synthesis_plan.json` was signed after the first search run for this topic, the script writes outputs to `results/<TOPIC>/synthesis/post_hoc/` instead and prints a note — say so explicitly to the reviewer and cite that path, not `synthesis/`, in later steps.)
...
5. For every group that clears the gate: calls `heterogeneity.compute_heterogeneity` (Q, df, p-value, I² — descriptive only, never used to pick a model), then `pooling.pool_effects` twice: once with the model prespecified in `synthesis_plan.json` (the primary, reported model) and once more with the other model (reported as `effect_sizes.json`'s `sensitivity` field for that outcome). Both calls also use `synthesis_plan.json`'s `tau2_estimator` (`"dl"` DerSimonian-Laird or `"pm"` Paule-Mandel) and `ci_method` (`"normal"` or `"hksj"`, modified Hartung-Knapp-Sidik-Jonkman) — echoed into each pooled entry's `tau2_estimator`/`ci_method` fields; cite whichever was used when drafting Methods, and surface `pooled_effect.ci_caution` verbatim in Results if `ci_method="hksj"` flagged one (fewer than 5 studies contributing). Every pooled entry also carries `model_source` (`"protocol"` if the plan was signed before the first search run, `"post_hoc"` if it was signed afterward, `"cli_override"` if `--model` was passed) — cite this when drafting Methods/Results, never present a `post_hoc` or `cli_override` model as if it had been prespecified. A random-effects pool with k≥3 also carries a 95% prediction interval (`pooled_effect.pi_low`/`pi_high`, human-readable as `display_pi_low`/`display_pi_high`) for a new study's true effect — report it alongside the CI when discussing how much a future study's result might plausibly differ, never conflate it with the confidence interval for the mean.
6. Renders a forest plot (`plots.forest_plot`, one row per study plus the pooled diamond, plus a prediction-interval bar when one was computed) for every pooled group, written to `results/<TOPIC>/synthesis/forest_<outcome-slug>.svg`. **Note the deliberate generalization from plan §6's illustrative single `forest_plot.svg`**: a review with multiple pooled outcomes needs one forest plot per outcome, so the actual filenames are per-outcome-slug and their paths are recorded in `effect_sizes.json`'s `forest_plot_svg`/`funnel_plot_svg` fields — this is what to cite when drafting Results/Figures in `/litreview-report`, not a hardcoded filename. Forest plots are drawn on the **pooling scale** (log scale for OR/RR, natural scale for RD/MD/SMD) with the null line at 0 — the human-readable exponentiated numbers (for OR/RR) live alongside in each entry's `display_estimate`/`display_ci_low`/`display_ci_high`/`display_pi_low`/`display_pi_high` fields for prose and tables, not on the plot axis itself.
...
- One line noting that `/litreview-report` will read these files verbatim for Methods §2.9/2.12, Results §3.3-3.7, and the Discussion's certainty framing — no re-elicitation needed.

Do not paste plot SVG contents into the reply — reference the file paths, the same discipline `/litreview-screen` uses for screening sheets.
