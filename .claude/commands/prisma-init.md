...
5. On confirmation (G-Route): **stop this entire command** — do not proceed to Step 1 and do not write a `protocol.json` under `<TOPIC>`. Hand off directly to `/prisma-search --living <chosen review's topic>`, which computes the delta rerun's `--since` bound from that topic's last recorded `protocol.json.versions[]` entry (`tools/versioning.py --topic <chosen> latest`) and records a new version once it completes.
...
**Known carve-out** — if `matched_row == "R3"` **and** `"answer"` is in `goal` **and** `question_focus == "forming"`: this is a documented routing-table ordering defect (`methods/_routing.json`'s R3 row fires unconditionally on `question_focus == "forming"`, before R5 — which is what actually applies to this combination — ever gets evaluated; flagged since PR #30, not yet fixed at the table level because reordering it is a maintainer judgment call, not something to silently patch here). Now that `methods/reconnaissance.json` is shipped (`docs/ROADMAP.md` M4), this row **resolves** to a real, valid `reconnaissance` recommendation instead of refusing — which makes the defect more consequential, not less: a reviewer could accept a fully-rendered reconnaissance card without ever learning R5's intended `scoping_review` resolution existed for their answers. R5's intended resolution for exactly this combination is `scoping_review`, which **is** shipped. Do not render the reconnaissance card as if it were the only fit here — tell the reviewer plainly: "your question isn't focused enough yet to run as a systematic review; a quick reconnaissance brief would work, but this also fits a scoping review, which maps the space first and gives you more to build on" and offer both, letting the reviewer choose. Record whichever they pick via `record_override()` (set `chosen_id` explicitly either way, `override_reason` citing this R3/R5 ordering note) — never silently confirm the router's literal `recommended_id` for this specific combination without surfacing the choice.
