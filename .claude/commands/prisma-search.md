...
4. Parse the remaining flags in `$ARGUMENTS`, if any:
   ...
   - `--living` - a living-review delta rerun (docs/ROADMAP.md M5): same behavior as `--rerun` (same plan, no keyword or enablement changes), except Step 5's `--since` bound comes from this topic's last recorded `protocol.json.versions[]` entry instead of the protocol's original fixed `eligibility.date_range.from`, and Step 7's dedup pass records a new version once it completes. Requires `search_plan.json` to already exist (same as `--rerun`) **and** at least one prior version already recorded (`python3 tools/versioning.py --topic <TOPIC> latest` returns non-null) - if none exists, **stop** and say so: run a normal search first, then `python3 tools/versioning.py --topic <TOPIC> record` once it's screened, before living mode has a version boundary to compute a delta against. Mutually exclusive with `--revise-keywords`/`--change-sources`/`--chase-citations` - living mode is specifically "the same plan, just what's new since last time," never a structural change in the same run.
...
Write `results/<TOPIC>/rerun_search.sh` with exactly this content (this is the literal reproducibility artifact PRISMA Item 7 points to - it is regenerated by every `/prisma-search` run so it always matches the plan it claims to replay, never hand-edited in between). Substitute `__LIVING_FLAG__` below with the literal string `1` if this run was invoked with `--living`, or `0` otherwise - this is what makes the generated script honestly reflect which `--since` bound it actually replays (a living rerun's own last-recorded-version date, or the protocol's original fixed date range) rather than silently defaulting to one or the other:
...
# re-typed here. A living-mode delta rerun (docs/ROADMAP.md M5) bounds
# SINCE by this topic's last recorded version instead - re-searching from
# the review's original start date every time would re-fetch everything,
# not just what's new.
LIVING="__LIVING_FLAG__"
if [ "$LIVING" = "1" ]; then
  SINCE="$(python3 tools/versioning.py --topic "$(basename "$HERE")" latest | python3 -c "
import json, sys
v = json.load(sys.stdin)
print((v or {}).get('date') or '')
")"
else
  SINCE="$(python3 -c "
import json
...
## Step 7d: Record a version (living-mode runs only)

If this run was invoked with `--living`, close out the delta by recording a new version snapshot:

```bash
python3 tools/versioning.py --topic <TOPIC> record
```

(Pre-allowlisted - `Bash(python3 tools/versioning.py:*)`.) Prints the new version's number, date, record count, and included count. For a non-`--living` run, skip this step entirely - an ordinary build/revise/rerun never advances the version counter; only a living delta rerun closes a version boundary.
