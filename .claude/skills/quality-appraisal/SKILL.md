...
- **`/prisma-extract`**: for every included study, run the RoB2 six-domain assessment (RCTs) or the Newcastle-Ottawa Scale (non-randomized studies) and write the result into that study's `risk_of_bias` block in `extraction_table.json`.
...
Ask the reviewer to confirm study design during `/prisma-extract` if the included-study table doesn't already record it unambiguously — RoB2 and NOS are not interchangeable, and using RoB2's "allocation concealment" domain on a cohort study produces a meaningless judgement (there was no allocation to conceal).
...
- `01-risk-of-bias.md` — RoB2 six-domain definitions and judgement criteria (RCTs), the Newcastle-Ottawa Scale (non-randomized studies).
