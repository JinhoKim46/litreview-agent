...
- 2.8 Study risk of bias assessment [Item 11] — Name the tool (RoB2, per this framework's `quality-appraisal` skill) and describe the assessment process, drawn from `synthesis/rob_table.json`.
...
- 3.3 Risk of bias in studies [Item 18] — Present risk of bias assessments from `synthesis/rob_table.json`, alongside the traffic-light plot at `synthesis/rob_traffic_light.svg` **if `/prisma-synthesize` wrote one** (embed that file directly — it is already rendered from the real per-study RoB2 domain judgements; never regenerate or redraw it here). If it's absent (e.g. an all-NOS review, or RoB2 studies recorded with only an `overall_judgement` and no domain breakdown), present the summary table alone and say so in one line, not silently.
...
3. Use **WebSearch** (and **WebFetch** on a candidate source page, e.g. the publisher/DOI landing page) to verify that each reference is real. Flag any that cannot be verified — do not silently drop or silently keep an unverifiable reference.
4. Present corrections with brief explanations of what was wrong.

When generating references during drafting (e.g. citing the PRISMA 2020 statement itself, or citing methodological sources such as RoB2, GRADE, or a statistical method), always use WebSearch to find the real source first. Never fabricate any part of a reference. A study reference pulled directly from `extraction_table.json`/`records.jsonl` already carries a connector-verified DOI or URL — re-verify it only if its metadata looks incomplete or inconsistent, not as a blanket re-check.
...
- PRISMA is a **reporting** guideline, not a **conduct** guideline. It tells you what to report, not how to do the review. This framework's other commands (`/prisma-init`, `/prisma-search`, `/prisma-screen`, `/prisma-extract`, `/prisma-synthesize`) handle the conduct; this skill handles the reporting.
